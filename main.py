    ...
    for i in range(0,2*maxT):
        for j in range(0,int(numberOfTasks)):
            if(i==Task_Matrix[j][5]):
                ...
                gnt.arrow(i, 10*(int(numberOfTasks)-Task_Matrix[j][6]+1), 0, 4.5, head_width=0.2, head_length=0.5, fc='k', ec='k')
                gnt.arrow(i+Task_Matrix[j][1], 10*(int(numberOfTasks)-Task_Matrix[j][6]+1), 0, -4.5, head_width=0.2, head_length=0.5, fc='r', ec='r')  
        ...
        if(len(stack)!=0):
            ...
            if(len(stack)!=0):
                ...
                if(i>=current_task[5]-current_task[2]+current_task[1]):
                    gnt.broken_barh([(i, 1)], (10*(int(numberOfTasks)-current_task[6]+1)-4, 8), facecolors =('tab:red'))
                    isSchedulable = False
                else:
                    gnt.broken_barh([(i, 1)], (10*(int(numberOfTasks)-current_task[6]+1)-4, 8), facecolors =('tab:blue'))
    ...
    for i in range(0,2*maxT):
        for j in range(0,int(numberOfTasks)):
            if(i==Task_Matrix[j][5]):
                Task_Matrix[j][5] = Task_Matrix[j][5]+Task_Matrix[j][2]
                gnt.arrow(i, 10*(int(numberOfTasks)-Task_Matrix[j][6]+1), 0, 4.5, head_width=0.2, head_length=0.5, fc='k', ec='k')
                gnt.arrow(i+Task_Matrix[j][3], 10*(int(numberOfTasks)-Task_Matrix[j][6]+1), 0, -4.5, head_width=0.2, head_length=0.5, fc='r', ec='r') 
                stack.append(list(Task_Matrix[j]))
